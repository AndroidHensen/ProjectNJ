...
public class ReturnOfficialTaskAdapter extends BaseAdapter {
    private List<ReturnProjectTask> lists = new ArrayList<>();
    private Context context;

    public ReturnOfficialTaskAdapter(Context context, List<ReturnProjectTask> lists) {
        this.lists = lists;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ReturnOfficialTaskAdapter.ViewHolder holder = null;
        if(convertView==null){
            ...
            holder=(ReturnOfficialTaskAdapter.ViewHolder)convertView.getTag();
        }
        holder.data.setText(lists.get(position).getData());
        holder.data.setTextColor(context.getResources().getColor(R.color.blue));
        holder.detail.setText("完成进度："+lists.get(position).getProgress());
